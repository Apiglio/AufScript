...
type
  ...
  TAufScript = class
    ...
    published
      procedure send_error(str:string; error_type:TAufScriptError=AufsErr_Unknown);
...
begin
  ...
  if AAuf.ArgsCount<3 then filename:=''
    ...
    try
      ...
      AufScpt.send_error('警告：第二个参数需要是字符串变量，语句未执行。',AufsErr_Unknown);exit;
    end;
  end;
  case lowercase(mode) of
    ...
    '-ocp':AufScpt.occupied_ram_export(filename);
      ...
      try
        ...
        AufScpt.send_error('警告：第一个参数若不是-all或-ocp，则需要是ARV变量，语句未执行。',AufsErr_Unknown);exit;
      end;
  end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：参数需要是字符串变量，语句未执行。',AufsErr_Unknown);exit;
  end;
  ...
  begin
    try
      ...
      if ((AAuf.ArgsCount<4) or (lowercase(AAuf.args[3])<>'-f')) and (tmp.size<>FileSize(filename)) then
        AufScpt.send_error('警告：ARV变量大小与文件大小不同，如需强制导入需要有-f参数，语句未执行。',AufsErr_Unknown)
      else
        AufScpt.arv_ram_import(tmp,filename);
    except
      AufScpt.send_error('警告：参数需要是ARV变量，语句未执行。',AufsErr_Unknown);exit;
    end;
  end;
end;
...
begin
  ...
  //{$elseif defined(UNIX)}
    ...
    //{$endif}
  {$else}
  AufScpt.send_error('当前系统不支持beep',AufsErr_PlatformUnimplemented);
  {$endif}
end;
...
begin
  ...
  {$else}
  AufScpt.send_error('非windows平台未实现cmd指令',AufsErr_PlatformUnimplemented);
  {$endif}
end;
...
begin
  ...
  case lowercase(AAuf.args[0]) of
    ...
    else AufScpt.send_error('未知函数，需要echo或echoln。',AufsErr_NoFunction);
  end;
end;
...
begin
  ...
  case lowercase(AAuf.args[0]) of
    ...
    'println': new_lined:=true;
      ...
      AufScpt.send_error('未知函数，需要print或println。',AufsErr_NoFunction);
      exit;
    end;
  end;
...
begin
  ...
  case lowercase(AAuf.args[0]) of
    ...
    else AufScpt.send_error('未知函数，需要hex或hexln。',AufsErr_NoFunction);
  end;
end;
...
begin
  ...
  case lowercase(AAuf.args[0]) of
    ...
    else AufScpt.send_error('未知函数，需要bin或binln。',AufsErr_NoFunction);
  end;
end;
          ...
          ARV_Float:
            begin
              case tmp.size of
                ...
                else AufScpt.send_error('暂不支持4bytes和8bytes以外的浮点数赋值。',AufsErr_PlatformUnimplemented);
              end;
            end;
        end;
      END;
    ELSE AufScpt.send_error('警告：mov的第一个参数有误，赋值未成功。',AufsErr_Unknown);
  end;
end;
...
begin
  ...
  if tmp1.size=0 then begin
    AufScpt.send_error('警告：add_arv的第1个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  tmp2:=AufScpt.RamVar(AAuf.nargs[2]);
  if tmp2.size=0 then begin
    AufScpt.send_error('警告：add_arv的第2个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
    ...
    ARV_Float:
      begin
        ...
        case tmp1.size of
          ...
          else AufScpt.send_error('警告：浮点数长度不支持。',AufsErr_PlatformUnimplemented);
        end;
      end;
    ARV_FixNum:ARV_add(tmp1,tmp2);
    else AufScpt.send_error('警告：add_arv的第1个参数类型不支持加法，代码未执行。',AufsErr_ParamType);
  end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：add的第二个参数不能转换为double类型，语句未执行。',AufsErr_Convert);
    exit;
  end;
  try
    ...
    AufScpt.send_error('警告：add的第一个参数需要是Float或FixNum变量，语句未执行。',AufsErr_ParamType);
    exit;
  end;
end;
...
begin
  ...
  tmp1:=AufScpt.RamVar(AAuf.nargs[1]);
  if tmp1.size=0 then begin
    AufScpt.send_error('警告：sub_arv的第1个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  tmp2:=AufScpt.RamVar(AAuf.nargs[2]);
  if tmp2.size=0 then begin
    AufScpt.send_error('警告：sub_arv的第2个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  case tmp1.VarType of
    ARV_Float:
    ...
    else AufScpt.send_error('警告：add_arv的第1个参数类型不支持减法，代码未执行。',AufsErr_ParamType);
  end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：sub的第二个参数不能转换为double类型，语句未执行。',AufsErr_Convert);
    exit;
  end;
  try
    a:=AufScpt.TryToDouble(AAuf.nargs[1]);
    ...
    AufScpt.send_error('警告：sub的第一个参数需要是Float或FixNum变量，语句未执行。',AufsErr_ParamType);
    exit;
  end;
end;
...
begin
  ...
  tmp1:=AufScpt.RamVar(AAuf.nargs[1]);
  if tmp1.size=0 then begin
    AufScpt.send_error('警告：mul_arv的第1个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  tmp2:=AufScpt.RamVar(AAuf.nargs[2]);
  if tmp2.size=0 then begin
    AufScpt.send_error('警告：mul_arv的第2个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  case tmp1.VarType of
    ARV_Float:
    ...
    else AufScpt.send_error('警告：add_arv的第1个参数类型不支持乘法，代码未执行。',AufsErr_ParamType);
  end;
end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：mul的第二个参数不能转换为double类型，语句未执行。',AufsErr_Convert);
    exit;
  end;
  try
    a:=AufScpt.TryToDouble(AAuf.nargs[1]);
    ...
    AufScpt.send_error('警告：mul的第一个参数需要是Float或FixNum变量，语句未执行。',AufsErr_ParamType);
    exit;
  end;
end;
...
begin
  ...
  if ARV_EqlZero(tmp2) then begin
    AufScpt.send_error('警告：div_arv的第2个参数不能为0，语句未执行。',AufsErr_DividedByZero);
    exit;
  end;
  ...
  case tmp1.VarType of
    ...
    else AufScpt.send_error('警告：add_arv的第1个参数类型不支持除法，代码未执行。',AufsErr_ParamType);
  end;
end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：div的第二个参数不能转换为double类型，语句未执行。',AufsErr_Convert);
    exit;
  end;
  if b=0 then begin AufScpt.send_error('警告：div的第二个参数不能为0，语句未执行。',AufsErr_DividedByZero);exit end;
  try
    a:=AufScpt.TryToDouble(AAuf.nargs[1]);
    tmp:=AufScpt.RamVar(AAuf.nargs[1]);
    case tmp.VarType of
    ...
    AufScpt.send_error('警告：div的第一个参数需要是Float或FixNum变量，语句未执行。',AufsErr_ParamType);
    exit;
  end;
end;
...
begin
  ...
  tmp1:=AufScpt.RamVar(AAuf.nargs[1]);
  if tmp1.size=0 then begin
    AufScpt.send_error('警告：mod_arv的第1个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  tmp2:=AufScpt.RamVar(AAuf.nargs[2]);
  if tmp2.size=0 then begin
    AufScpt.send_error('警告：mod_arv的第2个参数错误，语句未执行。',AufsErr_Unknown);
    exit;
  end;
  if ARV_EqlZero(tmp2) then begin
    AufScpt.send_error('警告：mod_arv的第2个参数不能为0，语句未执行。',AufsErr_DividedByZero);
    exit;
  end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：mod的第二个参数不能转换为double类型，语句未执行。',AufsErr_Convert);
    exit;
  end;
  if r=0 then begin AufScpt.send_error('警告：mod的第二个参数不能为0，语句未执行。',AufsErr_DividedByZero);exit end;
  try
    ...
    AufScpt.send_error('警告：mod的第一个参数需要是Float或FixNum变量，语句未执行。',AufsErr_ParamType);
    exit;
  end;
end;
...
begin
  ...
  if AAuf.ArgsCount>2 then begin
    ...
    if rand_max=0 then begin AufScpt.send_error('警告：第二个参数不能为0，语句未执行。',AufsErr_DividedByZero);exit end;
    case tmp.VarType of
...
begin
  ...
  with AufScpt do begin
    if (addr<0) or (addr>=ScriptLines.Count) then send_error('无效的行计数器。',AufsErr_AddressInvalid)
  else
...
begin
  ...
  if ofs=AufScpt.currentLine then begin AufScpt.send_error('警告：jmp需要非零的地址偏移量，该语句未执行。',AufsErr_AddressRecursive);exit end;
...
begin
  ...
  if not AAuf.CheckArgs(2) then exit;
  if not AAuf.TryArgToAddr(1,ofs) then exit;
  if ofs=AufScpt.currentLine then begin AufScpt.send_error('警告：call需要非零的地址偏移量，该语句未执行。',AufsErr_AddressRecursive);exit end;
...
begin
  ...
  if addr=0 then begin
    AufScpt.send_error('无效的loop跳转参数，loop必须跳转到init或标签行的下一行。',AufsErr_AddressInvalid);
    exit;
  end;
...
procedure _fend(Sender:TObject);
var AufScpt:TAufScript;
begin
  AufScpt:=Sender as TAufScript;
  with AufScpt do begin
    if PSW.stack_ptr=0 then begin
      send_error('警告：已在栈顶，该语句未执行。',AufsErr_StackOverflow);
      exit;
    end;
    if PSW.stack[PSW.stack_ptr].scriptname = PSW.stack[PSW.stack_ptr-1].scriptname then begin
      AufScpt.send_error('警告：栈顶与下一层是同一脚本文件，请使用ret，该语句未执行。',AufsErr_StackInvalid);
      exit;
    end;
    //ScriptLines.Free; //使用ScriptLinesMap不自动释放脚本
    AufScpt.pop_addr;
  end;
end;

procedure _ret(Sender:TObject);
var AufScpt:TAufScript;
begin
  AufScpt:=Sender as TAufScript;
  with AufScpt do begin
    ...
    if PSW.stack[PSW.stack_ptr].scriptname <> PSW.stack[PSW.stack_ptr-1].scriptname then begin
      send_error('警告：栈顶与下一层不是同一脚本文件，请使用fend，该语句未执行。',AufsErr_StackInvalid);
      exit;
    end;
    pop_addr;
  end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('警告：重命名宏定义名'+dn1+'为'+dn2+'时出错',AufsErr_ConflictDefName);
  end;
end;
...
begin
  ...
  try
    ...
    if tmpAEU=nil then begin
      AufScpt.send_error('警告：找不到宏定义'+defname,AufsErr_DefNameNotFound);
    end else begin
      case tmpAEU.value.pre of
        ...
        else AufScpt.send_error('警告：宏定义'+defname+'不是内存地址，无法修改内存类型',AufsErr_RamVarInvalid);
      end;
    end;
  except
    AufScpt.send_error('警告：创建宏定义'+defname+'的副本时出错',AufsErr_ConflictDefName)
  end;
end;
...
begin
  ...
  if AufScpt.Expression.Local.Find(var_name)<>nil then
    begin
      AufScpt.send_error('警告：变量已存在，该语句未执行。',AufsErr_ConflictDefName);
      exit
    end;
  if AAuf.ArgsCount>3 then begin
...
begin
  ...
  for arg_idx:=1 to AAuf.ArgsCount-1 do begin
    ...
    if tmp=nil then begin
      AufScpt.send_error('警告：找不到'+var_name+'的定义，该语句未执行。',AufsErr_Unknown);
      exit;
    end;
    arv:=AufScpt.RamVar(tmp.value);
    if arv.size=0 then begin
      AufScpt.send_error('警告：'+var_name+'不是ARV变量，该语句未执行。',AufsErr_Unknown);
      exit
    end;
    try
      AufScpt.Expression.Local.Remove(tmp);
    except
      AufScpt.send_error('警告：删除变量'+var_name+'时出错',AufsErr_Unknown)
    end;
...
begin
  ...
  IF AufScpt.Time.Synthesis_Mode=SynMoTimer THEN BEGIN
    ...
    {$endif}
  END ELSE BEGIN
    AufScpt.send_error('SynMoDelay时间模式不支持跨任务协同。',AufsErr_PlatformUnimplemented);
  END;
end;
...
begin
  ...
  for pi:=2 to AAuf.ArgsCount-1 do begin
    ...
    AufScpt.send_error('第'+IntToStr(pi)+'个参数'+AAuf.args[3]+'无法转换成字符串。',AufsErr_Convert);
  end;
  s_to_arv(res,arv);
end;
...
begin
  ...
  if tmpAEU.readonly then begin
    AufScpt.send_error('警告：变量为只读变量，不能进行指向修改。',AufsErr_Unknown);
    exit;
  end;
  arv:=AufScpt.RamVar(tmpAEU.value);
  if arv.size=0 then begin
    AufScpt.send_error('警告：变量不是合法指针类型，该语句未执行。',AufsErr_Unknown);
    exit;
  end;
  ...
  case func of
    ...
    'ptrmr':
      ...
      begin
        AufScpt.send_error('未知的指针偏移函数',AufsErr_NoFunction);
        exit;
      end
  end;
...
ErrOver_O:
  AufScpt.send_error('指针定义长度过小[O]。',AufsErr_Unknown);
  exit;
ErrOver_L:
  AufScpt.send_error('指针定义位移超界[L]。',AufsErr_Unknown);
  exit;
ErrOver_R:
      ...
      '-r','-rename':
        begin
          ...
          if rename_series<10000 then
            ...
            AufScpt.send_error(filename+'文件及其所有替代文件名均已存在，导出图片失败。',AufsErr_RunTime);
            result:='';
          end;
          exit;
        end;
        ...
        begin
          AufScpt.send_error(filename+'文件已存在，导出图片失败。',AufsErr_RunTime);
          result:='';
          exit;
        end;
    end;
...
begin
  ...
  if obj is TARImage then begin
    ...
    AufScpt.send_error('找不到对应的ARImage，删除失败',AufsErr_ParamType);
  end;
end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('图片导出失败，原因未知。',AufsErr_RunTime);
  end;
end;
...
begin
  ...
  try
    ...
    AufScpt.send_error('图片导入失败，原因未知。',AufsErr_RunTime);
  end;
end;
...
begin
  ...
  if new_img<>nil then begin
    ...
    AufScpt.send_error('裁切图像失败。',AufsErr_RunTime);
  end;
end;
...
begin
  if Self.ArgsCount < MinCount then
    begin
      Self.Script.send_error('警告：'+Self.nargs[0].arg+'需要（至少）'+IntToStr(MinCount-1)+'个变量，该语句未执行。',AufsErr_ParamCount);
      result:=false;
    end
  else result:=true;
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为字节byte，代码未执行。',AufsErr_Numeric);exit
  end;
  result:=true;
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为双字dword，代码未执行。',AufsErr_Numeric);exit
  end;
  result:=true;
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为双字DWord（有符号），代码未执行。',AufsErr_Numeric);exit
  end;
  result:=true;
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为字符串string，代码未执行。',AufsErr_Unknown);exit
  end;
  result:=true;
end;
...
begin
  ...
  Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不在['+list+']范围内，代码未执行。',AufsErr_NoNamedParam);
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为浮点型double，代码未执行。',AufsErr_Numeric);exit
  end;
  result:=true;
end;
...
begin
  ...
  try
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化为pRam('+AufScript_CPU+'整型数)，代码未执行。',AufsErr_ParamSize);exit
  end;
  result:=true;
end;
...
begin
  ...
  if not (res.VarType in TypeAllowance) then
    begin
      Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数类型有误，代码未执行。',AufsErr_ParamType);
      exit
    end;
  if maxsize>minsize then begin
    if (res.size>maxsize) or (res.size<minsize) then
      begin
        Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数变量长度应在'+IntToStr(minsize)+'至'+IntToStr(maxsize)+'范围内，而不能是'+IntToStr(res.size)+'，代码未执行。',AufsErr_ParamSize);
        exit
      end;
  end else if maxsize=minsize then begin
    if res.size<>maxsize then
      begin
        Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数变量长度应为'+IntToStr(maxsize)+'，而不能是'+IntToStr(res.size)+'，代码未执行。',AufsErr_ParamSize);
        exit
      end;
  end;
  result:=true;
...
begin
  ...
  if not (res[1] in ['_','A'..'Z','a'..'z']) then begin
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个宏定义名的第1个字符需要是字母或下划线，该语句未执行。',AufsErr_InvalidDefName);
    result:=false;
...
begin
  ...
  try
    case Self.nargs[ArgNumber].pre of
      ...
      ':':begin
        if not Script.ScriptLines.FindLabel(nargs[ArgNumber].arg, res) then begin
          Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数的标签'+nargs[ArgNumber].arg+'未找到，代码未执行。',AufsErr_AddressNotFound);
          exit;
        end else res:=res+1;
      end;
    ...
    Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数不能转化绝对地址，代码未执行。',AufsErr_AddressInvalid);exit
  end;
  result:=true;
end;
...
begin
  try
    ...
    if tmpBase is TAufArray then begin
      ...
      arr:=nil;
      result:=false;
      Script.send_error('警告：第'+IntToStr(ArgNumber)+'个参数无法正确解析为TAufArray实例，代码未执行。',AufsErr_Unknown);
    end;
  except
    arr:=nil;
    result:=false;
...
begin
  ...
  try
    ...
    Self.send_error('警告：文件写入失败！请检查'+filename+'是否被占用。',AufsErr_FileIOFailed);
  end;
end;
...
begin
  ...
  try
    ...
    Self.send_error('警告：文件写入失败！请检查'+filename+'文件是否被占用。',AufsErr_FileIOFailed);
    fs.Free;exit
  end;
  fs.Free;
end;
...
begin
  try
    ...
    Self.send_error('警告：文件读取失败！请检查'+filename+'是否存在或被占用。',AufsErr_FileIOFailed);
    fs.free;exit
  end;
...
begin
  with Self.Func_process.OperatorCompare do begin
    if Find(operator_name, index) then send_error('错误：已有运算符"'+operator_name+'"，不能覆盖定义！',AufsErr_DefTime)
    else begin
...
begin
  with Self.Func_process.OperatorCompare do begin
    if Find(operator_name, index) then begin
      ...
      send_error('错误：无效运算符"'+operator_name+'"，对比结果默认返回false！',AufsErr_NoOperator);
      is_error:=true;
      result:=false;
    end;
  end;
...
begin
  ...
  if (i=func_range-1) and (Self.func[i].name<>'') then begin
    Self.send_error('错误：函数列表已满，不能继续添加新的函数！',AufsErr_DefTime);
    exit;
  end;
...
begin
  ...
  repeat
    ...
    inc(i);
  until (i=func_range);
  Self.send_error('警告：未找到函数'+func_name+'！',AufsErr_NoFunction);
end;
            ...
            if not ScriptLines.FindLabel(labelName, line) then begin
              ...
              while line < ScriptLines.Count do begin
                ...
                if cmd = labelname+':' then begin
                  if not ScriptLines.AddLabel(labelName, line) then send_error('出现同名标签，新的标签行将被忽略。',AufsErr_EncTime);
                  break;
                end;
                inc(line);
              end;
...
begin
  if Self.PSW.stack_ptr=0 then
    begin
      Self.send_error('错误：[-1]超出栈范围！',AufsErr_StackOverflow);
      Self.PSW.haltoff:=true;
    end;
  Self.currentline:=0;
...
begin
  if Self.PSW.stack_ptr=stack_range-1 then
    begin
      Self.send_error('错误：['+Usf.to_s(stack_range)+']超出栈范围！',AufsErr_StackOverflow);
      Self.PSW.haltoff:=true;
    end;
